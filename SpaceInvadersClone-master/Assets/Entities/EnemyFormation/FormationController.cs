...
public class FormationController : MonoBehaviour {
    ...
    public GameObject jefes; //	GameObject to instantiate the enemy's ships
	...
	void SpawnEnemies() {
		...
		foreach (Transform child in transform) {
			GameObject enemy = Instantiate (fantasmaPrefab, child.transform.position, Quaternion.identity) as GameObject;
	...
	void SpawnUntilFull() {
		...
		//	free position.
		if (freePosition) {
			GameObject enemy = Instantiate (fantasmaPrefab, freePosition.position, Quaternion.identity) as GameObject;
	...
	void Update () {
		...
		if (AllMembersDead ()) {
			SpawnUntilFull2 ();	//	Previously SpawnEnemies ();
		}
	}

    private void SpawnUntilFull2()
    {
        foreach (Transform child in transform)
        {
            GameObject enemy2 = Instantiate(Fantasma2, child.transform.position, Quaternion.identity) as GameObject;
            enemy2.transform.parent = child;
            Transform freePosition = NextFreePosition();
        }
